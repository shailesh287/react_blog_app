                        ...
                        <div className="top-post-heading">Top Posts</div>
                        {
                            data.filter(
                                (latestArticle) => latestArticle.id === '2' && (latestArticle.category === 'Tourism' || latestArticle.category === 'Food' || latestArticle.category === 'Technology' || latestArticle.category === 'Bollywood' || latestArticle.category === 'Fitness')).map(
                                    (article) => <>
                                        <Link to={`/category/article/${article.id}/${article.category}`} style={{ textDecoration: "none" }} className="top-post-container">
                                            <img src={article.url} className="top-post-image" alt=""/>
                                            <div className="top-post-content">
                                                <h4 className="top-post-title">{article.title}</h4>
                                                <p>
                                                    <span className="top-post-category">{article.category}</span> / {article.date}
                                                </p>
                                            </div>
                                        </Link>
