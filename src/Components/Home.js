            ...
            <div className="home-container">
                <div className="image-container">
                    <div className="image-1">
                        {
                            data.filter(
                                (image) => image.id === '1' && (image.category === 'Tourism')).map(
                                    (image) =>
                                        <Link to={`/category/article/${image.id}/${image.category}`} style={{ textDecoration: "none" }} className="">
                                            ...
                                            <p className="image-text2">{image.category} / {image.date}</p>
                                        </Link>
                                )
                        }
                    </div>
                    <div className="image-2">
                        {
                            data.filter(
                                (image) => image.id === '1' && (image.category === 'Fitness' || image.category === 'Technology')).map(
                                    (image) =>
                                        <Link to={`/category/article/${image.id}/${image.category}`} style={{ textDecoration: "none" }} className="">
